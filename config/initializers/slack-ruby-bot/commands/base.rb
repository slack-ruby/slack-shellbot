    ...
    class Base
      class << self
        ...
        def invoke(client, data)
          ...
          client._say(as_user: true, channel: data.channel, text: "```#{e.document.errors.first[1]}```", gif: 'error')
          true
        rescue StandardError => e
          logger.info "#{name.demodulize.upcase}: #{client.owner}, #{e.class}: #{e}"
          client._say(as_user: true, channel: data.channel, text: "```#{e.message}```", gif: 'error')
          true
        end
      end
    end
