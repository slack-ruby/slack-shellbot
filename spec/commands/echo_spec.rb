...
describe SlackShellbot::Commands::Echo do
  ...
  context 'echo' do
    ...
    it 'pipe into a file' do
      expect do
        expect(client).to receive(:chat_postMessage).with(channel: 'channel', text: '```hi```', as_user: true)
        expect(client).to receive(:chat_postMessage).with(channel: 'channel', text: '```2 byte(s) written```', as_user: true)
