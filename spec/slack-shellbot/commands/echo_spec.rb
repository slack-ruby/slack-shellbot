...
describe SlackShellbot::Commands::Echo do
  ...
  context 'echo' do
    it 'no message' do
      ...
      message_hook.call(client, text: "#{SlackRubyBot.config.user} echo", channel: 'channel')
    end
    it 'message' do
      expect(client).to receive(:say).with(channel: 'channel', text: 'hi')
      message_hook.call(client, text: "#{SlackRubyBot.config.user} echo hi", channel: 'channel')
    end
    it 'pipe into a file' do
      expect do
        ...
        message_hook.call(client, text: "#{SlackRubyBot.config.user} echo hi > \"text file.txt\"", channel: 'channel')
