  ...
  context 'vi' do
    it 'creates a file' do
      expect do
        ...
        message_hook.call(client, Hashie::Mash.new(team: team, text: "#{SlackRubyBot.config.user} vi test.txt", channel: 'channel'))
    ...
    context 'existing file' do
      ...
      it 'opens an existing file' do
        expect do
          ...
          message_hook.call(client, Hashie::Mash.new(team: team, text: "#{SlackRubyBot.config.user} vi \"#{file.name}\"", channel: 'channel'))
        end.to change(Program, :count).by(1)
        expect(fs.reload.program).to be_a ViProgram
      end
    end
    context 'opened file' do
      ...
      it 'appends to the file' do
        ...
        message_hook.call(client, Hashie::Mash.new(team: team, text: "#{SlackRubyBot.config.user} another line", channel: 'channel'))
      ...
      it 'saves file' do
        ...
        message_hook.call(client, Hashie::Mash.new(team: team, text: "#{SlackRubyBot.config.user} :wq", channel: 'channel'))
      ...
      it 'quits without saving file' do
        ...
        message_hook.call(client, Hashie::Mash.new(team: team, text: "#{SlackRubyBot.config.user} :q", channel: 'channel'))
        expect(file.reload.data).to eq ['hello world'].join("\n")
      end
    end
  end
